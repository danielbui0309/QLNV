...
public class PaytypeDAO {
    ...
    public List<Paytype> list() {
        ...
        if (cache.size() != 0 && usingCache) {
            for (Map.Entry<Integer, Paytype> obj : cache.entrySet()) {
        ...
        try {
            ...
            Paytype.PaytypeBuilder builder;
            Paytype paytype;

            while (rs.next()) {
                builder = new Paytype.PaytypeBuilder();
        ...
        try {
            ...
            if (result != 0) {
                ...
                while (rs.next()) {
                    ...
                    Paytype.PaytypeBuilder builder = new Paytype.PaytypeBuilder();
                    builder.setId(rs.getInt("id"));
                    builder.setTitle(rs.getString("title"));
                    builder.setDate(rs.getTimestamp("date"));
    ...
    private boolean createControl(Paytype paytype) {
        for (Map.Entry<Integer, Paytype> obj : cache.entrySet()) {
            if (obj.getValue().getTitle().equals(paytype.getTitle())) {
                DB.ERROR_MESSAGE = obj.getValue().getTitle() + " THE RECORD HAS ALREADY EXISTED!";
                return false;
            }
        }
        return true;
