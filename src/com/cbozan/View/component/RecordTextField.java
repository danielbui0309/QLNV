...
public class RecordTextField extends JTextField implements FocusListener {
    ...
    public static final int IBAN_NUMBER_TEXT = 6; //Belongs to worker
    ...
    private final String phoneNumberRegex = "^((((\\+84)?|(0)?)\\d{9})|())$";
    private final String requiredPhoneNumberRegex = "^(((\\+84)?|(0)?)\\d{9})$";
    //Số thẻ tín dụng bao gồm 16 số
    private final String ibanNumberRegex = "^((\\d{16})|())$";
    private final String requiredIbanNumberRegex = "^((\\d{16}))$";
    //Dùng để tính tiền lương hoặc các số decimal khác mà mình đã thiết lập trong database
    ...
    public RecordTextField(int regexNumber) {
        if(regexNumber == NON_REQUIRED_TEXT) {
        ...
        } else if(regexNumber == DECIMAL_NUMBER_TEXT || regexNumber == DECIMAL_NUMBER_TEXT + NON_REQUIRED_TEXT) {
